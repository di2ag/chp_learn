...
class Gene(models.Model):
    ...
    fill_g2g_results = models.ManyToManyField('Gene', through='GeneFillToGeneResult', related_name='g2g_filled_by')
    fill_dis2g_results = models.ManyToManyField('Disease', through='DiseaseFillToGeneResult', related_name='dis2g_filled_by')
...
class Disease(models.Model):
    curie = models.CharField(max_length=128, unique=True)
    name = models.CharField(max_length=128, null=True)
    fill_g2dis_results = models.ManyToManyField('Gene', through='GeneFillToDiseaseResult', related_name='g2dis_filled_by') 

    def __str__(self):
        return str(self.curie)

class GeneFillToGeneResult(models.Model):
...
class DiseaseFillToGeneResult(models.Model):
...
class GeneFillToDiseaseResult(models.Model):
